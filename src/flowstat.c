...
/* Used for log flow stats to file */
struct flow_log {
 ...
 */
struct lognode {
	struct flow_log *info;		/* Node content */
...
static void format_log(char *buf, struct flow_log *flow_log)
{
	struct dp_session_info *info = &flow_log->session;
	time_t utc_now = mktime(gmtime(&flow_log->timestamp));
	struct tm *ts = localtime(&flow_log->timestamp);
	...
	sprintf(buf,
		...
		flow_log->duration, info->se_app_name ? info->se_app_name : "",
...
static void queue_log(struct flow_log *flow_log)
{
			    ...
			    enum dp_session_state state)
{
	...
	struct flow_log *flow_log = malloc(sizeof(struct flow_log));
	struct dp_session_info *info = &flow_log->session;
			     ...
			     void *data __unused)
{
	...
	if (debug_level >= DEBUG_LEVEL) {
		if (hook == SESSION_STATS_UPDATE) {
			char msg[LOG_MSG_SIZE];
			struct flow_log flow_log;
			dp_session_query(s, SESSION_ATTR_ALL,
					 &flow_log.session);
			format_log(msg, &flow_log);
